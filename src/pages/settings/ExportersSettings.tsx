...
const ExportersSettings: React.FC = () => {
    ...
    const fetchExporters = async () => {
        try {
            setLoading(true);
            const response = await exportersAPI.getAll();
    ...
    const handleAddSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingId) {
                await exportersAPI.update(editingId, formData);
            } else {
                await exportersAPI.create(formData);
            }
            ...
            alert('Failed to save exporter');
        }
    };

    const handleEdit = (exporter: any) => {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            setImporting(true);
            const res = await exportersAPI.import(formData);
    ...
    const handleDelete = async (id: string) => {
        if (!window.confirm('Are you sure you want to delete this exporter?')) return;
        try {
        ...
        <div className="flex-1 flex flex-col h-full bg-white">
            <div className="px-8 py-8 flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">Manage your exporter directory</p>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-8 pb-8 custom-scrollbar">
                {loading ? (
                    <div className="text-center py-10 text-gray-500">Loading exporters...</div>
                    ...
                    <div className="flex flex-col items-center justify-center h-64 text-center border-2 border-dashed border-gray-200 rounded-lg bg-gray-50/50">
                        ...
                        <h3 className="text-lg font-medium text-gray-900">No exporters found</h3>
                        <p className="text-sm text-gray-500 mt-1 max-w-sm">
                            {(searchTerm) ? 'Try adjusting your search terms.' : 'Get started by importing an Excel sheet or adding exporters manually.'}
                        </p>
                    </div>
                ) : (
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
                        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                            <h3 className="font-bold text-lg text-gray-900">{editingId ? 'Edit Exporter' : 'Add New Exporter'}</h3>
                        ...
                        <form onSubmit={handleAddSubmit} className="p-6 space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="e.g. Global Exports"
                                />
                            </div>
                            <div>
                            ...
                            <div className="pt-2 flex justify-end gap-3">
                                ...
                                >
                                    {editingId ? 'Save Changes' : 'Create Exporter'}
                                </button>
                            </div>
                        </form>
                    </div>
...
export default ExportersSettings;
