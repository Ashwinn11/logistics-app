...
const SearchableSelect: React.FC<SearchableSelectProps> = ({
    ...
    return (
        <div className={`relative ${className}`} ref={wrapperRef}>
            ...
            {/* Native Select for Required validation (hidden but focusable for validation messages?) */}
            {/* Actually opacity 0 input prevents validation bubble visibility usually. But better than nothing. */}
            <input
                value={value}
                required={required}
                onChange={() => { }}
            ...
            {isOpen && (
                <div className="absolute z-20 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-60 overflow-y-auto animate-fade-in">
